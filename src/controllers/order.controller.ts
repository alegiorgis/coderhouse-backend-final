...
export class OrderController {
    ...
    public getOrderById: RequestHandler = async (req, res) => {
        ...
        try {
            ...
            if (id) {
               ...
               if (!response) {
                   throw new GenericException({ status: 404, message: 'Orden no encontrado.' });
               }
            } else {
            ...
            throw new GenericException({ status: 500, message: 'Error interno de servidor.' });
        }
    }
    ...
    public updateOrder: RequestHandler = async (req, res) => {
        ...
        try {
            ...
            if (!order) {
                throw new GenericException({ status: 404, message: 'Ordero no encontrado.' });
            } else {
