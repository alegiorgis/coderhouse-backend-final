...
export class CartService {
    ...
    async getAllProductsByCartId(id: string) {
        ...
        if (!cart) {
            throw new GenericException({ status: 404, message: `Carrito con id ${id} no encontrado.` });
        } 
     ...
     async addProductsToCart(cartId: string, products: Product[]) {
        ...
        if (!response) {
            throw new GenericException({ status: 404, message: `Id de carrito ${cartId} no encontrado.` });
        }
        return response;
     }
     ...
     async createOrder(cartId: string, products: Product[], name: string,
