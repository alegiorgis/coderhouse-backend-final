...
export class AuthService {
    ...
    private cartDao: CartDaoMongo | CartDaoFirebase | CartDaoFs;
    ...
    // login
    async login(username: string, password: string, completion: (userRes: any) => void) {
             ...
             bcrypt.compare(password, user.password, function(err, result) {
                 if (result) {
                     jwt.sign(
                         ...
                         (err, token) => {
                            if (err) {
                                throw new GenericException({ status: 404, message: 'Token generation error' })
                            }
                     ...
                     throw new GenericException({ status: 402, message: 'Invalid credentials.' })
                 }
             });       
         } else {
    ...
    async register(userBody: User) {
        ...
        const self = this;
        if (existsUser) {
            throw new GenericException({ status: 404, message: 'User already exists.' })
        } else { 
